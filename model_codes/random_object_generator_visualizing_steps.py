...
if __name__ == '__main__':
    num_points1 = 7 
    num_points2 = 5
    filled_mask = generate_filled_object2(num_points1, num_points2, array_size=(100, 100), smoothing_factor=0.5)
    blurred_mask = gaussian_filter(filled_mask.astype(float), sigma=4)

    plt.imshow(blurred_mask, cmap='gray')  # Display the image in grayscale
    plt.axis('off')  # Remove the ticks and axis
    plt.show()

    # # Apply Gaussian blurring
    # blurred_mask = gaussian_filter(filled_mask.astype(float), sigma=2)

    # # Visualize side by side
    # fig, axes = plt.subplots(1, 2, figsize=(10, 5))
    
    # axes[0].imshow(filled_mask, cmap='gray')
    # axes[0].set_title("(a) Random Lesion Sharp Edge")
    # axes[0].axis('off')

    # axes[1].imshow(blurred_mask, cmap='gray')
    # axes[1].set_title("Smoothed Lesion")
    # axes[1].axis('off')
